...
docker run -d --rm --network flower --name logserver flowerpytorch:latest \
  python3 -m flwr_experimental.logserver

docker run -d --rm --network flower --name server flowerpytorch:latest \
...
docker run -d --rm --network flower --name client_0 flowerpytorch:latest \
  python3 -m flwr_example.cifar_app.client --cid=0 --server_address=server:8080 --log_host=logserver:8081

docker run -d --rm --network flower --name client_1 flowerpytorch:latest \
