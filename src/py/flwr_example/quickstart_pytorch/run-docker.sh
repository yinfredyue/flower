...
docker run -d --rm --network flower --name server pytorchcpu:latest python3 -m flwr_example.quickstart_pytorch.server --log_host=logserver:8081
...
docker run -d --rm --network flower --name client_0 pytorchcpu:latest \
  python3 -m flwr_example.quickstart_pytorch.client --cid=0 --nb_clients=4 --server_address=server:8080 --log_host=logserver:8081

docker run -d --rm --network flower --name client_1 pytorchcpu:latest \
  python3 -m flwr_example.quickstart_pytorch.client --cid=1 --nb_clients=4 --server_address=server:8080 --log_host=logserver:8081

docker run -d --rm --network flower --name client_2 pytorchcpu:latest \
  python3 -m flwr_example.quickstart_pytorch.client --cid=2 --nb_clients=4 --server_address=server:8080 --log_host=logserver:8081

docker run -d --rm --network flower --name client_3 pytorchcpu:latest \
  python3 -m flwr_example.quickstart_pytorch.client --cid=3 --nb_clients=4 --server_address=server:8080 --log_host=logserver:8081
