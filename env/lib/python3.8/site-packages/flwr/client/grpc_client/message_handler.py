...
from flwr.proto.transport_pb2 import ClientMessage, Reason, ServerMessage, FitIns, ReadRes, ReadIns
...
def handle(
    ...
    if server_msg.HasField("read_res"):
        return _read(client, server_msg.read_res), 0, True
    if server_msg.HasField("update_stale_ins"):
        return _update_staleness(client, server_msg.update_stale_ins), 0, True
...
def _fit(client: Client, fit_msg: FitIns) -> ClientMessage:
    """_fit could generate FitRes or ReadIns.
    If the cached parameter is fresh enough, FitRes. Otherwise, ReadIns to pull parameter from server.
    """
    ...
    if isinstance(reply, typing.FitRes):
        # Serialize fit result
        fit_res_proto = serde.fit_res_to_proto(reply)
        return ClientMessage(fit_res=fit_res_proto)
    elif isinstance(reply, typing.ReadIns):
        # Serialize read ins
        read_ins_proto = serde.read_ins_to_proto(reply)
        return ClientMessage(read_ins=read_ins_proto)
    else:
        raise Exception("This shouldn't happen")


def _read(client: Client, read_res_msg: ReadRes) -> ClientMessage:
    read_res = serde.read_res_from_proto(read_res_msg)
    fit_res = client.read(read_res)
    fit_res_proto = serde.fit_res_to_proto(fit_res)
    return ClientMessage(fit_res=fit_res_proto)


def _update_staleness(client: Client, update_stale_ins_msg: ServerMessage.UpdateSIns) -> ClientMessage:
    update_stale_ins = serde.stale_ins_from_proto(update_stale_ins_msg)
    client.update_staleness(update_stale_ins)
