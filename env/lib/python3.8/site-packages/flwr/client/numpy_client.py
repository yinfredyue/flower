...
from flwr.common.chaos import fixed_delay
...
class NumPyClientSSPWrapper(Client):
    def __init__(self, numpy_client: NumPyClient, staleness_bound: int = 2) -> None:
        self.numpy_client = numpy_client
...
class NumPyClientSSPStragglerWrapper(NumPyClientSSPWrapper):

    def fit(self, ins: FitIns) -> Union[FitRes, ReadIns]:
        res = super().fit(ins)
        if isinstance(res, ReadIns):
            return res
        
        fixed_delay(20)             # Adjust delay to test different straggler situations
        return res
