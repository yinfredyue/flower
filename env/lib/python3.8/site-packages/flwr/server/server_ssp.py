...
class ServerSSP:
    ...
    def fit_client(self, client: ClientProxy, num_rounds: int, staleness_bound: int):
        for current_round in range(1, num_rounds):
            ...
            if isinstance(reply, ReadIns):
                ...
                read_res = ReadRes(weights_to_parameters(weights), fit_ins)
    ...
    def update_clock(self, fit_res):
        with self.lock:
            ...
            for cid, clock in self.client_clocks:
    ...
    def register_client(self, cid) -> bool:
        with self.lock:
            if cid in self.client_clocks:
                return False

            self.client_clocks[cid] = -1
            return True
