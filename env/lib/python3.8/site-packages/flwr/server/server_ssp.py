...
class ServerSSP(ServerBase):
    def __init__(
        ...
        self.eval_interval = timedelta(minutes=1) # 1-minute interval
    ...
    def fit_client(self, client: ClientProxy, num_rounds: int, staleness_bound: int):
        ...
        for current_round in range(0, num_rounds):
            ...
            # Evaluate for 2 cases: 1. clock updated; 2. interval reached.
            need_eval = False
            ...
            with self.lock:
                ...
                if global_clock_updated:
                    need_eval = True
                    self.prev_eval_time = datetime.now()
                    log(DEBUG, "time to evaluate. Clock updated")
                ...
                if weights_prime is not None:
                    ...
                    if self.prev_eval_time is None or datetime.now() > self.prev_eval_time + self.eval_interval:
                        need_eval = True
                        self.prev_eval_time = datetime.now()
                        log(DEBUG, "time to evaluate. Evaluation interval")
                else:
                    log(WARN, "weight_prime is None ???")

                # Evaluate model using strategy implementation, if global clock is updated
                # Centralized evaluation

            # Not holding lock
            # evaluate model for every update
            if need_eval:
                res_cen = self.strategy.evaluate(weights=weights_prime)

                with self.lock:
                    if res_cen is not None:

                        if self.sp_strategy is not None:
                            self.sp_strategy.feed(res_cen[1])
            ...
            # Send server's staleness bound to the client
            if self.sp_strategy is not None:
                update_stale_ins = UpdateSIns(self.staleness_bound)
