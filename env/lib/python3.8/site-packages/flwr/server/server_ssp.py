...
def compare_weights(w1: Weights, w2: Weights):
    same = 0
    diff = 0
        ...
        for current_round in range(0, num_rounds):
            ...
            with self.lock:
                ...
                if weights_prime is not None:
                    same, diff = compare_weights(self.weights, weights_prime)
                    if same > 0:
                        log(WARN, f"{same} parameter layers very similar in total {len(self.weights)}")
            ...
            with self.lock:
                if res_cen is not None:
                    if global_clock_updated and self.sp_strategy is not None:
                    ...
                    # log(
                    #     INFO,
                    #     u"\u001b[31;1m[Clock {:03d}]\u001b[32m Loss: {:.3f}, Accuracy: {:.3%}, Total time elapsed: {:.3f}\u001b[0m".format(
                    #         self.global_clock + 1,
                    #         loss_cen,
                    #         acc_cen,
