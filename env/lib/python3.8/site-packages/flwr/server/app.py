...
from flwr.server.server_ssp import ServerSSP
...
def start_server(
    ...
    Arguments:
        ...
        server: Optional[flwr.server.Server] (default: None). An implementation
            ...
            provided, then `start_server` will create one BSP server.
...
def start_server_ssp(
    staleness_bound: int,
    num_clients: int,
    server_address: str = DEFAULT_SERVER_ADDRESS,
    server: ServerSSP = None,
    config: Optional[Dict[str, int]] = None,
    strategy: Optional[Strategy] = None,
    grpc_max_message_length: int = GRPC_MAX_MESSAGE_LENGTH,
    switchpoint_strategy: SPStrategy = None
) -> None:
    if server is None:
        client_manager = SimpleClientManager()
        if strategy is None:
    ...
    start_server(server_address, server, config, strategy, grpc_max_message_length)
