...
class GrpcClientProxy(ClientProxy):
    ...
    def fit(self, ins: common.FitIns) -> Union[common.ReadIns, common.FitRes]:
        ...
        if client_msg.HasField("fit_res"):
            fit_res = serde.fit_res_from_proto(client_msg.fit_res)
            return fit_res
        elif client_msg.HasField("read_ins"):
            read_ins = serde.read_ins_from_proto(client_msg.read_ins)
            return read_ins
        else:
    ...
    def read(self, ins: common.ReadRes) -> common.FitRes:
        """Accept fresh parameters and finish the paused fit operation."""
        read_res_msg = serde.read_res_to_proto(ins)
        client_msg: ClientMessage = self.bridge.request(
            ServerMessage(read_res=read_res_msg)
        )
        fit_res_msg = serde.fit_res_from_proto(client_msg.fit_res)
        return fit_res_msg

    def update_staleness(self, ins: common.UpdateSIns) -> None:
        """Update staleness bound"""
        update_stale_msg = serde.stale_ins_to_proto(ins)
        self.bridge.request(ServerMessage(update_stale_ins=update_stale_msg))
