...
from flwr.proto.transport_pb2 import ClientMessage, Parameters, Reason, ServerMessage, ReadIns, ReadRes, FitIns
...
def fit_ins_to_proto(ins: typing.FitIns) -> FitIns:
    ...
    return FitIns(parameters=parameters_proto, config=ins.config, clock=ins.clock)


def fit_ins_from_proto(msg: FitIns) -> FitIns:
    """Deserialize flower.FitIns from ProtoBuf message."""
    parameters = parameters_from_proto(msg.parameters)
    config = dict(msg.config)
    return typing.FitIns(clock=msg.clock, parameters=parameters, config=config)
...
def read_res_to_proto(ins: typing.ReadRes) -> ReadRes:
    """Serialize flower.ReadRes to ProtoBuf message."""
    parameters_proto = parameters_to_proto(ins.parameters)
    return ReadRes(
        parameters=parameters_proto,
        fit_ins=fit_ins_to_proto(ins.fit_ins),
        clock=ins.clock
    )

def read_ins_to_proto(ins: typing.ReadIns) -> ReadIns:
    return ReadIns(
        min_clock=ins.min_clock,
        fit_ins=fit_ins_to_proto(ins.fit_ins),
    )

def read_res_from_proto(res: ReadRes) -> typing.ReadRes:
    parameters = parameters_from_proto(res.parameters)
    return typing.ReadRes(
        parameters=parameters,
        fit_ins=fit_ins_from_proto(res.fit_ins),
        clock=res.clock,
    )

def read_ins_from_proto(ins: ReadIns) -> typing.ReadIns:
    return typing.ReadIns(
        min_clock=ins.min_clock,
        fit_ins=fit_ins_from_proto(ins.fit_ins),
    )

#  === Update staleness bound message ===

def stale_ins_to_proto(s_ins: typing.UpdateSIns) -> ServerMessage.UpdateSIns:
    """Serialize UpdateSIns to ProtoBuf message."""
    return ServerMessage.UpdateSIns(staleness_bound=s_ins.staleness_bound)

def stale_ins_from_proto(msg: ServerMessage.UpdateSIns) -> typing.UpdateSIns:
    """Serialize UpdateSIns to ProtoBuf message."""
    return typing.UpdateSIns(staleness_bound=msg.staleness_bound)
